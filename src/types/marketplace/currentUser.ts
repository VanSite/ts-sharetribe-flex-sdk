...
type CurrentUserTypeType<P extends AllCurrentUserParameter> =
  'include' extends keyof P ? (P['include'] extends CurrentUserRelationshipsFields[] ? true : false) : false;

type IncludedType<P extends AllCurrentUserParameter> =
  'include' extends keyof P ? (
    P['include'] extends (keyof RelationshipTypeMap)[]
      ? Array<RelationshipTypeMap[P['include'][number]]> : never
    ) : never;

type ExpandReturnType<P extends AllCurrentUserParameter, EP> =
  EP extends { expand: true } ? CurrentUserType<CurrentUserTypeType<P>> :
    EP extends { expand: false } ? Omit<CurrentUserType<CurrentUserTypeType<P>>, 'attributes'> :
...
> =
  E extends 'show' ? CurrentUserType<CurrentUserTypeType<P>> :
    E extends 'delete' ? Pick<DeleteCurrentUser, 'id' | 'type'> :
      E extends 'create' | 'createWithIdp' | 'updateProfile' | 'changePassword' | 'changeEmail' | 'verifyEmail' ? ExpandReturnType<P, EP> :
        never
