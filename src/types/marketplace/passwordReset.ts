...
type ExpandReturnType<EP> =
  EP extends { expand: true } ? PasswordReset :
    EP extends { expand: false } ? Omit<PasswordReset, 'attributes'> :
...
> =
  E extends 'request' ? ExpandReturnType<EP> :
    E extends 'reset' ? ExpandReturnType<EP> :
      never;
