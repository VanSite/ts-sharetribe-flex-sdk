...
import { ApiParameter, ExtraParameter, UUID, Relationship, RelationshipTypeMap, ExtraParameterType } from '../sharetribe';
...
type StripeCustomerTypeType<P extends AllStripeCustomerParameter> =
  'include' extends keyof P ? (P['include'] extends StripeCustomerRelationshipsFields[] ? true : false) : false;
...
type ExpandReturnType<P extends AllStripeCustomerParameter, EP> =
  EP extends { expand: true } ? StripeCustomerType<StripeCustomerTypeType<P>> :
    EP extends { expand: false } ? Omit<StripeCustomerType<StripeCustomerTypeType<P>>, 'attributes'> :
...
> =
  E extends 'create' ? ExpandReturnType<P, EP> :
    E extends 'addPaymentMethod' ? ExpandReturnType<P, EP> :
      E extends 'deletePaymentMethod' ? ExpandReturnType<P, EP> : never;
