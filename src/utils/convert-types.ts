 ...
 */
export function convertToLatLngBounds(
...
export function convertTypes(key: string, value: any): any {
  ...
  if (
    ["price", "payinTotal", "payoutTotal", "lineTotal", "unitPrice"].includes(
      key
    )
  ) {
 ...
 */
export function recursiveApplyHandler(
  ...
  if (typeHandlers) {
    typeHandlers.forEach((handler) => {
      recursiveApplyHandler(
        data,
        ({ key, value }: { key: string; value: any }) => {
          if (handler.sdkType && handler.reader) {
            // @ts-ignore - Using instanceof with constructor function
            if (value instanceof handler.sdkType) {
              return handler.reader(value);
            }
          }

          if (handler.canHandle && handler.reader) {
            if (handler.canHandle({ key, value })) {
              return handler.reader(value);
            }
          }
