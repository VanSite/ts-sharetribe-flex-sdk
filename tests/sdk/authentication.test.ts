...
import MemoryStore from "../../src/utils/stores/memory-store";
...
describe('Authentication process', () => {
  ...
  describe('Non-trusted user', () => {
    ...
    it('should get auth info for a user that is logged in', async () => {
      mockAdapter.onPost('https://flex-api.sharetribe.com/v1/auth/token').reply(200, {
        access_token: 'test-access-token',
        token_type: 'bearer',
        expires_in: 86400,
        scope: 'public-read',
      })
      mockAdapter.onGet('https://flex-api.sharetribe.com/v1/api/marketplace/show').reply(200, {
        data: {
    ...
    it('should logout the user', async () => {
      ...
      mockAdapter.onPost('https://flex-api.sharetribe.com/v1/auth/revoke').reply(200, {
        revoked: true
      })
  ...
  describe('Trusted user', () => {
    ...
    it('should login the user via Idp', async () => {
      ...
      const trustedMockAdapter = new AxiosMockAdapter(trustedSharetribeSdk.axios)

      trustedMockAdapter.onPost('https://flex-api.sharetribe.com/v1/auth/auth_with_idp').reply(200, {
